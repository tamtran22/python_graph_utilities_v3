...
def edge_to_node(
    ...
    if len(edge_attr.shape) <= 1:
        _node_attr = np.zeros(shape=(number_of_nodes,), dtype=np.float32)
    else:
        _node_attr = np.zeros(shape=(number_of_nodes, number_of_attrs), dtype=np.float32)
    for i in range(number_of_edges):
        _node_attr[edge_index[0][i]] = edge_attr[i]
    ##
    _edge_has_parrent = np.isin(edge_index[1], edge_index[0])
    _root_edge = np.where(_edge_has_parrent == False)[0][0]
    _node_attr[edge_index[1][_root_edge]] = edge_attr[_root_edge]
